...
const GraphVisualizer = ({
	...
	useEffect(() => {
		...
		const theme = {
			...
			selection: rootStyles.getPropertyValue('--viz-selection-color').trim(),
			...
			textOnNode: rootStyles.getPropertyValue('--color-text-on-node').trim() || 'white',
		...
		const nodes = graph.nodes.map(node => {
			...
			return {
				...
				color: {
					...
					highlight: {background: theme.selection, border: theme.primary},
					hover: {background: theme.nodeFill, border: theme.primary},
				},
		...
		const edges = sortedEdges.map(edge => {
			...
			if (isSelfLoop) {
				...
				if (edge.from < edge.to) {
					smoothConfig = {enabled: true, type: 'curvedCW', roundness: 0.3};
				}
			}
